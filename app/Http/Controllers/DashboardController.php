...
class DashboardController extends Controller
{
    ...
    private function admin($user, $satuan): View
    {
        ...
        $distribusiPenggunaKategori = $semuaSatuan->groupBy('kategori')->map(fn ($group, $kategori) => [
            'kategori' => $labelKategori[$kategori] ?? ucfirst($kategori),
            'jumlah' => $group->sum('users_count'),
        ])->values();
        $statusResetPassword = collect($permintaanResetPassword)->groupBy('status')->map(fn ($group, $status) => [
            'status' => $status,
            'jumlah' => $group->count(),
        ])->values();
        ...
        $sesiAktif = DB::table('sessions')->leftJoin('users', 'sessions.user_id', '=', 'users.id')
            ->orderByDesc('sessions.last_activity')
            ->get(['sessions.id','sessions.ip_address','sessions.user_agent','sessions.last_activity','users.name as user_name']);
        $rekapLaporanSatuan = Satuan::where('kategori', Satuan::KATEGORI_SATLAK)->withCount([
        ...
        return view('siberad.dashboards.admin', [
            'user' => $user, 'satuan' => $satuan, 'semuaPengguna' => $semuaPengguna,
            'semuaSatuan' => $semuaSatuan, 'permintaanResetPassword' => $permintaanResetPassword,
            'distribusiPenggunaKategori' => $distribusiPenggunaKategori, 'statusResetPassword' => $statusResetPassword,
            'kelengkapanAkunSatuan' => ['sudah' => $satuanSudahPunyaAkun, 'belum' => $satuanBelumPunyaAkun],
            'pengaturan' => Pengaturan::current(), 'logAktivitas' => $logAktivitas,
            'daftarBackup' => $daftarBackup, 'daftarPengumuman' => $daftarPengumuman,
            'sesiAktif' => $sesiAktif, 'sesiSayaId' => session()->getId(),
            'rekapLaporanSatuan' => $rekapLaporanSatuan, 'modulHakAkses' => Satuan::MODUL_HAK_AKSES,
            'stats' => [
                'total_pengguna' => $semuaPengguna->count(), 'total_satuan' => $semuaSatuan->count(),
    ...
    private function pelaporan($user, $satuan, ?string $kode): View
    {
        ...
        $canReview = in_array($kode, ['DANPUS', 'WADAN'], true);
        $canSend = $kode !== 'DANPUS';
        ...
        $description = match ($kode) {
            ...
            'WADAN' => 'Monitoring dan review laporan antar satuan sebagai bagian dari koordinasi sebelum diteruskan.',
        ...
        $monitoringSatlak = collect();
        $laporanSatlak = collect();
        if ($mode === 'duktek') {
            ...
            $monitoringSatlak = Satuan::whereIn('kode', ['SATLAKKAL', 'SATLAKSISOS', 'SATLAKDAK'])->orderBy('urutan')->get()->map(function ($satlak) use ($laporanSatlak) {
                return ['nama' => $satlak->nama, 'total' => $laporanSatlak->where('satuan_id', $satlak->id)->count()];
            });
        }

        return view('siberad.dashboards.laporan-role', [
            'user' => $user, 'satuan' => $satuan, 'tujuan' => $tujuan,
            'defaultTujuanId' => $defaultDanpus?->id, 'laporanTerkirim' => $laporanTerkirim,
            'laporanMasuk' => $laporanMasuk, 'laporanSatlak' => $laporanSatlak,
            'monitoringSatlak' => $monitoringSatlak, 'mode' => $mode, 'canReview' => $canReview,
            'canSend' => $canSend, 'description' => $description,
            'stats' => [
                'dikirim' => $laporanTerkirim->count(),
                'menunggu' => $laporanTerkirim->where('status', 'Menunggu')->count(),
                'disetujui' => $laporanTerkirim->filter(fn ($l) => str_contains(strtolower((string) $l->status), 'setuj'))->count(),
                'ditolak' => $laporanTerkirim->filter(fn ($l) => str_contains(strtolower((string) $l->status), 'tolak'))->count(),
                'masuk' => $laporanMasuk->count(),
            ],
        ]);
    }
}
